    ...
    <section id="certificates" data-testid="certificates-section" className="relative py-14 md:py-20 px-6 md:px-12">
      <div className="max-w-7xl mx-auto">
            ...
            {CERTS.map((c, i) => {
              ...
              return (
                <motion.button
                  ...
                  viewport={{ once: true }}
                  whileHover={{ y: -16, rotate: delta * 11, x: delta * 140, scale: 1.04, zIndex: 50 }}
                ...
                >
                  <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `linear-gradient(to top, rgba(5, 5, 5, 0.95) 0%, rgba(5, 5, 5, 0.4) 50%, transparent 100%), url(${c.img})` }}>
                    <div className="absolute bottom-5 left-5 right-5 text-left">
                      <span className="text-[10px] tracking-[0.24em] uppercase mb-1 block font-semibold" style={{ color: c.accent }}>{c.org}</span>
                      <div className="font-serif text-lg text-white leading-tight mt-1">{c.title}</div>
                    </div>
                  </div>
                </motion.button>
              );
        ...
        <div className="md:hidden grid grid-cols-1 sm:grid-cols-2 gap-4">
          {CERTS.map((c, i) => (
            <button key={c.id} onClick={() => setOpen(c)} className="relative rounded-2xl overflow-hidden border border-white/10 bg-[#0d0d0d] aspect-[4/3]">
              <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `linear-gradient(to top, rgba(5, 5, 5, 0.95) 0%, rgba(5, 5, 5, 0.4) 50%, transparent 100%), url(${c.img})` }}>
                <div className="absolute bottom-4 left-4 right-4 text-left">
                  <span className="text-[9px] tracking-[0.24em] uppercase mb-1 block font-semibold" style={{ color: c.accent }}>{c.org}</span>
      ...
      <AnimatePresence>
        {open && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setOpen(null)} className="fixed inset-0 z-[100] bg-[#050505]/95 backdrop-blur-md grid place-items-center p-4 md:p-10">
            <motion.div initial={{ scale: 0.92, y: 30 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.92, y: 20 }} transition={{ duration: 0.4 }} onClick={(e) => e.stopPropagation()} className="relative max-w-4xl w-full rounded-2xl overflow-hidden border border-white/15 bg-[#0d0d0d]">
              <div className="w-full h-[55vh] md:h-[65vh] bg-black relative flex items-center justify-center border-b border-white/5">
                <iframe src={`https://google.com{open.pdf}&embedded=true`} title={open.title} className="w-full h-full border-0" />
              </div>
              <button onClick={() => setOpen(null)} className="absolute top-3 right-3 w-9 h-9 grid place-items-center rounded-full bg-black/60 border border-white/10 text-white hover:bg-black/80 transition-colors z-50">
                <XIcon size={16} />
              </button>
              <div className="px-5 py-4 flex items-center justify-between gap-4 bg-[#0d0d0d]">
                ...
                <a href={open.pdf} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full bg-white text-black px-4 py-2 text-[12px] font-medium hover:bg-neutral-200 transition-colors z-50" onClick={(e) => e.stopPropagation()}>
