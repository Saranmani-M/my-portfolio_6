...
const SiLinux = svgIcon("0 0 24 24", "M12.504 0c-.155 0-.315.008-.480.021-4.226.333-3.105 4.807-3.17 6.298-.076 1.092-.3 1.953-1.005 3.02-.806 1.207-1.937 2.7-2.875 4.314-.932 1.59-1.893 3.33-2.393 5.103-.498 1.768-.952 3.656-.parameters 5.103.003 1.092.532 2.224 1.424 2.765.899.54 2.09.475 3.313.052 1.264-.437 2.602-.836 3.993-.918.703-.04 1.405-.04 2.108 0 1.391.082 2.729.481 3.993.918 1.223.423 2.414.488 3.313-.052.892-.54 1.421-1.673 1.424-2.765-.002-1.447-.455-3.335-.953-5.103-.5-1.773-1.461-3.513-2.393-5.103-.938-1.614-2.069-3.107-2.875-4.314-.705-1.067-.929-1.928-1.005-3.02-.065-1.491 1.056-5.966-3.17-6.298-.164-.013-.324-.021-.479-.021z");
...
const SiGit = svgIcon("0 0 24 24", "M21.62 11.108l-8.731-8.729a1.294 1.294 0 00-1.823 0L9.257 4.19l2.299 2.3a1.532 1.532 0 011.939 1.95l2.214 2.217a1.532 1.532 0 011.784 2.handling 1.533 1.533 0 01-1.517 1.527 1.53 1.53 0 01-1.517-1.527v-.012a1.529 1.529 0 00-.994-1.429L11.61 9.61v4.09a1.533 1.533 0 11-1.51.001V9.36a1.533 1.533 0 01-.449-2.861L7.397 4.19 1.379 10.208a1.294 1.294 0 000 1.822l8.731 8.729a1.294 1.294 0 001.822 0l9.689-9.652a1.294 1.294 0 000-1.999");

const SiDocker = svgIcon("0 0 24 24", "M4.5 10.5v2h2v-2zm0 2v2h2v-2zm0 2v2h2v-2zm0 2v-2h-2v2zm2-8v2h2v-2zm0 2v2h2v-2zm0 2v2h2v-2zm0 2v2h2v-2zm2-8v2h2v-2zm0 2v2h2v-2zm0 2v2h2v-2zm0 2v2h2v-2zm2-8v2h2v-2zm0 2v2h2v-2zm0 2v2h2v-2zm0 2v2h2v-2zm2-8v-2h-2v2zm0 2v-2h2v2zm0 2v-2h2v2zm.7c-.3-.2-1-.4-1.7-.3-.3-.2-.8-.5-1.1-.6-.5-.2-1-.2-1.5 0-.5.2-1 .5-1.2 1-.6-.5-1.1-1.1-1.1-1.5l-3-.2-1.5 1.1-1.6-.5-1.1.6-.5 1.1.5 1.1 1.5-.5");

const SiPycharm = svgIcon("0 0 24 24", "M4 2h16v9.51-8 6.3-8-6.3V2zm2 2h12v6.416 4.7-6.4V4zm0 10v2h6.416 4.7-6.4v-2zm-2 4h12v2H4zm4-8h2v4H8v-4zm4-2h2v6h-2v-6zm4 2h2v4h-2v-4z");
...
const BubbleBackground = () => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const cx = width * 0.5;
      const cy = height * 0.5;
      const baseRadius = Math.max(width, height) * 0.48;
      const cosA = Math.cos(t);
      const sinA = Math.sin(t);
      const cosB = Math.cos(0.15);
      ...
      projected.forEach((p) => {
        const depth = (p.z + 1.3) / 2.6;
        const glow = ctx.createRadialGradient(p.sx, p.sy, 0, p.sx, p.sy, p.r * 2.2);
        ...
        const body = ctx.createRadialGradient(lightX, lightY, p.r * 0.05, p.sx, p.sy, p.r * 1.15);
        const base = 8 + depth * 14;
